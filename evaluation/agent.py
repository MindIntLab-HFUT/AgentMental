...
for idx, evaluation in enumerate(data, 1):
    ...
    for key, value in evaluation.items():
        if key not in ["json_file", "report"] and isinstance(value, dict):
            ...
            dialogue_text += section_title

            
            if "question" in value and "answer" in value:
                dialogue_text += f"心理助手: {value['question']}\n用户: {value['answer']}\n"
            
            elif "qa_dialogue" in value:
            ...
            if "score" in value:
                dialogue_text += f"主题评分: {value['score']}\n"
            if "summary" in value:
                dialogue_text += f"主题得分依据: {value['summary']}\n"
